...
namespace Tiny_RenderPipeline
{
    ...
    public class CustomRenderPipeline : RenderPipeline
    {
       ...
       // Unity的默认表面着色器具有ForwardBase通道，该通道用作第一个正向渲染通道。我们可以使用它来识别具有与默认管道一起使用的材质的对象。
        ...
        void Render_01_sky_box(Camera camera, ScriptableRenderContext renderContext, int D_LightDir, int D_LightColor, int _CameraPos, int _PLightPos, int _PLightColor)
        {
            //将上下文设置为当前相机的上下文。
        ...
        protected void RenderAllCamera(ScriptableRenderContext renderContext, List<Camera> cameras)
        {
            ...
            foreach (var camera in cameras)
            {
               ...
               // Render_01_sky_box(camera, renderContext, D_LightDir, D_LightColor, _CameraPos, _PLightPos, _PLightColor);
            }
        }

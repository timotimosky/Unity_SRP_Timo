...
float GetOtherShadowAttenuation (OtherShadowData other, ShadowData global, Surface surfaceWS) 
{
	...
	float shadow;
	if (other.strength * global.strength <= 0.0) //global.strength<0 超出了阴影距离.跳过采样实时阴影
	{
		//global.shadowMask 级联
		//级联仅适用于定向阴影。它们对于其他光线没有意义，因为它们具有固定的位置，因此其阴影贴图不会随视图移动。
		//但是，将所有阴影都以相同的方式淡出是个好主意，否则我们可能会遇到屏幕上某些没有方向性阴影但确实有其他阴影的区域。
		//因此，我们将对所有事物使用相同的全局阴影强度。
		shadow = GetBakedShadow(global.shadowMask, other.shadowMaskChannel, abs(other.strength));
	}
	else
	{
		//我们必须处理的一个极端情况是当没有方向性阴影存在而我们确实有其他阴影。
		//这时不会有任何级联，因此它们不应影响全局阴影强度。而且我们仍然需要阴影距离淡入值。
		//因此，让我们移动代码来设置级联计数和从Shadows.RenderDirectionShadows到的距离衰减，
		//Shadows.Render并在适当时将级联计数设置为零。
